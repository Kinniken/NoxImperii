...
function Forma:assignCoords()
  ...
  if self.formation == "cross" then
   ...
   elseif self.formation == "trade column" then
      --This formation is a simple "|", with fleader in the lead.
      radius = 100
      for i = 1, numships do         
         angle = math.pi
         radius = 100 * i
...
function Forma:control()
   ...
   for i, p in ipairs(self.fleet) do
      if not (p == self.fleader) then
         ...
         -- local distance = cons:mod()

         -- if ((distance < 60 and mem.in_formation) or (distance < 20)) then
         --    p:faceSameDir(self.fleader)
         --    p:matchVelocity(self.fleader,500)

         --    mem.in_formation = true
         --    p:setSpeedLimit(self.fleetspeed)
         -- else
         --    p:setSpeedLimit( 0 )

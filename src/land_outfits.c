...
static void outfits_genList( unsigned int wid )
{
   ...
   if (noutfits <= 0) { /* No outfits */
      ...
      for (i=0; i<noutfits; i++) {
         ...
         /* Background colour. Removed in LN (replaced with layers) */
         /*
...
void outfits_update( unsigned int wid, char* str )
{
   ...
   char *outfitname;
   Outfit* outfit;
   char buf[PATH_MAX], buf2[ECON_CRED_STRLEN], buf3[ECON_CRED_STRLEN], buf4[PATH_MAX], buf5[PATH_MAX];
   double th;
   int iw, ih;
   int w, h;
   ...
   if (outfit->factionNeeded == 0)
	   strncpy( buf5, "None", sizeof(buf5) );
   else if (faction_getPlayer(outfit->factionNeeded) >= outfit->factionRelationNeeded)
   	   nsnprintf( buf5, sizeof(buf5), "%s (%i)\e0", faction_longname(outfit->factionNeeded),outfit->factionRelationNeeded);
   else
	   nsnprintf( buf5, sizeof(buf5), "\er%s (%i)\e0", faction_longname(outfit->factionNeeded),outfit->factionRelationNeeded);
...
int outfit_canBuy( char *name, Planet *planet )
{
   ...
   /* needs minimum rep */
   if (outfit->factionNeeded>0) {
	   if (faction_getPlayer(outfit->factionNeeded) < outfit->factionRelationNeeded) {
		   land_errDialogueBuild( "You need relations of at least %i with the %s to buy this outfit.",
				   outfit->factionRelationNeeded, faction_longname(outfit->factionNeeded) );
		   failure = 1;
	   }
   }

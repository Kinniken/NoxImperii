...
/*
 * player crew stack - crew he has (active or not)
 */
static HiredCrew *player_crews  	 = NULL;  /**< Crew player has. */
static int player_ncrews             = 0;     /**< Number of outfits player has. */
static int player_mcrews             = 0;     /**< Current allocated memory. */
#define CREW_CHUNKSIZE               32       /**< Allocation chunk size. */
...
static int player_saveShip( xmlTextWriterPtr writer,
		Pilot* ship, char* loc );
static int player_saveCrew( xmlTextWriterPtr writer,
...
{
	return player_rating_other(player.crating);
}

/**< Combat ratings. */
/**
 * @brief Get a provided combat rating in a human-readable string.
 *
 *    @return The provided combat rating in a human readable string.
 */
const char* player_rating_other (double combatRating)
{
	if (combatRating == 0.) return player_ratings[0];
	else if (combatRating < 25.) return player_ratings[1];
	else if (combatRating < 50.) return player_ratings[2];
	else if (combatRating < 100.) return player_ratings[3];
	else if (combatRating < 200.) return player_ratings[4];
	else if (combatRating < 500.) return player_ratings[5];
	else if (combatRating < 1000.) return player_ratings[6];
	else if (combatRating < 2000.) return player_ratings[7];
	else if (combatRating < 5000.) return player_ratings[8];
	else if (combatRating < 10000.) return player_ratings[9];
...
/**
 * @brief Returns the player's crew.
 *
 *    @param[out] n Number of distinct outfits (not total quantity).
 *    @return Outfits the player owns.
 */
...
/**
 * @brief Returns the hired crew of this type, if any
 *
...
HiredCrew* player_getCrew(const char* name )
{
	...
	for (i=0;i<player_ncrews;i++) {
		if (strcmp(player_crews[i].crew->name,name)==0) {
			return &player_crews[i];
		}
	}
...
/**
 * @brief sets the status (active/reserve) of a hired crew
 *
 * @param name of the crew type
 * @param status to set
...
/**
 * @brief Adds a crew to the player crew stack.
 *
 *    @param o Crew to add.
...
int player_addCrew( const Crew *crew, const char* generatedName, int messages, int active, int status )
{
	...
	/* Try to find it. */
	for (i=0; i<player_ncrews; i++) {
		if (player_crews[i].crew == crew) {
			return -1;//can only have one crew of each "type"
		} else if ( strcmp(player_crews[i].crew->position,crew->position)==0 && player_crews[i].active) {
			positionTaken=1;
		}
	}
	...
	/* Allocate if needed. */
	player_ncrews++;
	if (player_ncrews > player_mcrews) {
		if (player_mcrews == 0)
			player_mcrews = OUTFIT_CHUNKSIZE;
		else
			player_mcrews *= 2;
		player_crews   = realloc( player_crews,
				sizeof(HiredCrew) * player_mcrews );
	}
	...
	if (active==-1) {
		...
		setActive=!positionTaken;
	} else {
		player_crews[player_ncrews-1].active=active;
		setActive=active;
	}

	//refreshed player's stats if new guy is active
	if (setActive)
		pilot_calcStats( player.p );

	if (messages) {
		if (!setActive) {
			dialogue_msg("New Hire","%s joins your crew. As the %s position is already taken, %s is in reserve."
					" Use the 'Crew' tab to make %s active.",generatedName,crew->position,crew_getGenderPronoun(crew),generatedName);
		} else {
			dialogue_msg("New Hire","%s joins your crew. As the %s position was vacant, %s fills in the role from now onward."
								,generatedName,crew->position,crew_getGenderPronoun(crew));
		}
	}
...
/**
 * @brief Remove a crew from the player's crew stack.
 *
 *    @param crew Crew to remove.
...
int player_rmCrew( const Crew *crew )
{
	...
	for (i=0; i<player_ncrews; i++) {
		if (player_crews[i].crew == crew) {
			...
			player_ncrews--;
			memmove( &player_crews[i], &player_crews[i+1],
					sizeof(HiredCrew) * (player_ncrews-i) );
...
int player_save( xmlTextWriterPtr writer )
{
	...
	/* Crews */
	xmlw_startElem(writer,"crews");
	for (i=0; i<player_ncrews; i++)
		player_saveCrew( writer, player_crews[i]);
	xmlw_endElem(writer);
...
/**
 * @brief Saves a hired crew.
 *
 *    @param writer XML writer.
 *    @param crew Hired crew to save
 *    @return 0 on success.
 */
static int player_saveCrew( xmlTextWriterPtr writer,
		const HiredCrew hiredCrew )
{
	xmlw_startElem(writer,"crew");
...
static Planet* player_parse( xmlNodePtr parent )
{
	...
	do {
		...
		/* Parse crews. */
		else if (xml_isNode(node,"crews")) {
			cur = node->xmlChildrenNode;
			do {
				if (xml_isNode(cur,"crew"))
					player_parseCrew(cur);
			} while (xml_nextNode(cur));
		}
...
/**
 * @brief Parses a hired crew.
 *
 *    @param parent Node of the cre
 *    @return 0 on success.
 */
static int player_parseCrew( xmlNodePtr parent )
{
	...
	xmlr_attr(parent,"name",name);
	xmlr_attr(parent,"generatedName",generatedName);
